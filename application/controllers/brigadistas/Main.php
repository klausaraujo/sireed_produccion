...
class Main extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $funcion = $this->Brigadista_model->listaFuncionComision();
        //$lista = $this->Brigadista_model->listar();  
        $lista = $this->Brigadista_model->listarRenarhed();
        ...
        $this->load->view("brigadistas/Main", $data);
    }
    ...
    public function anucomisiones()
    {
        ...
        if ($this->Brigadista_model->anularcomision() == 1) {
            //$this->deleteFile($file);
            $this->session->set_flashdata('mensajeWarning', 'Registro Anulada con éxito.');
        } else {
    ...
    public function guardarComision() {
        ...
        $idrenarhed = $this->input->post("idrenarhed");
        $apellidos = $this->input->post("apellidos");
        $nombres = $this->input->post("nombres");
        $numero_documento = $this->input->post("numero_documento");
        $funcion = $this->input->post("idfuncion");
        ...
        if ($idcomision > 0) {
            if ($this->Brigadista_model->actualizarComision()) {
                $respuestaEliminar = $this->Brigadista_model->eliminarDetalleComision();
    ...
    public function guardarDetalleComision($idcomision, $idrenarhed, $apellidos, $nombres, $numero_documento, $funcion) {
        ...
        foreach($idrenarhed as $key => $id):
            $this->Brigadista_model->setIdrenarhedcomi($id);
            $this->Brigadista_model->setApellidoscomi($apellidos[$key]);
            $this->Brigadista_model->setNombrescomi($nombres[$key]);
            $this->Brigadista_model->setNro_Documentocomi($numero_documento[$key]);
            $this->Brigadista_model->setFuncioncomi($funcion[$key]);

            $this->Brigadista_model->guardarDetalleComision();
        endforeach;
    ...
    public function obtenerDetalleComision(){
        $this->load->model("Brigadista_model");
        $this->Brigadista_model->setIdcomision($this->input->post("id"));
        
        $listacomi = $this->Brigadista_model->obtenerDetalleListaComisiones();
        $detalle = array(
