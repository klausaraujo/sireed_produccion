...
function areasByUser(Anio_Ejecucion,Codigo_Sector,Codigo_Pliego,Codigo_Ejecutora,Codigo_Centro_Costos,Codigo_Sub_Centro_Costos,Codigo_Usuario) {
	...
	$.ajax({
		...
		method:'post',
		type:'json',
		data:{Anio_Ejecucion:Anio_Ejecucion,Codigo_Sector:Codigo_Sector,Codigo_Pliego:Codigo_Pliego,Codigo_Ejecutora:Codigo_Ejecutora,
...
$(document).ready(function(){
	...
	/*$("#cboAnioArea").change(function(){

		$("#areasModal").modal("show");
		var anio = $(this).val();
		var Codigo_Usuario = $("#Codigo_Usuario").val();
		if(anio.length>0 && Codigo_Usuario.length>0){

			areasByUser(anio,Codigo_Usuario);
